...
/** ✅ 추가: 거래처 타입 (SalesInput2에서 넘기는 형태랑 동일) */
export type Customer = {
  id: number;
  customerName: string;
};

export type SalesLine2 = {
  itemName: string;
  qty: number;
  price: number;
  discount: number;
...
type Props = {
  ...
  onClose: () => void;
  onSetSales: React.Dispatch<React.SetStateAction<Sales2>>;

  updateLine: (idx: number, patch: Partial<SalesLine2>) => void; // ✅ 이거 중요
  addLine: () => void;
  ...
  // ✅ ✅ ✅ 여기 추가해야 SalesInput2에서 customerList 넘길 수 있음
  customerList: Customer[];
};

export default function SalesModal2({
  show,
  selectedId,
  sales,
  onClose,
